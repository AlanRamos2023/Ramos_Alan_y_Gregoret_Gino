    ...
    internal class Program
    {
        ...
        static void LibrosMenu2()
        {
            ...
            do
            {
                ...
                Console.WriteLine("6. Actulizar Genero");
                Console.WriteLine("7. Mostrar Libros");
                Console.WriteLine("8. Mostrar generos");
                Console.WriteLine("9. Borrar Libro");
                Console.WriteLine("10. Volver al menú principal");
                Console.Write("Seleccione una opción: ");

                while (!int.TryParse(Console.ReadLine(), out subOpcion) || subOpcion < 1 || subOpcion > 10)
                {
                    Console.Write("Entrada inválida. Por favor, seleccione una opción entre 1 y 10: ");
                }
        ...
        static void MenuAgregarLibros()
        {
            ...
            using (var connection = new MySqlConnection(connectionString))
            {
                ...
                int idGenero = SeleccionarOpcionConAgregar(generos, "genero");

                Console.WriteLine("Selecciona un autor:");
                int idAutor = SeleccionarOpcionConAgregar(autores, "autor");
        ...
        static int SeleccionarOpcionConAgregar(List<(int Id, string Nombre)> opciones, string tipo)
        {
            ...
            Console.WriteLine($"{opciones.Count + 1}. Agregar nuevo {tipo}");

            int seleccion;
            while (true)
            {
                Console.Write("Selecciona una opción: ");
                if (int.TryParse(Console.ReadLine(), out seleccion) && seleccion > 0 && seleccion <= opciones.Count + 1)
                {
                    break;
                }
            ...
            {
                Console.Write($"Ingresa el nuevo {tipo}: ");
                string nuevoNombre = Console.ReadLine();

                if (tipo == "genero")
                {
                    return AgregarNuevoGenero(tipo, nuevoNombre);
                }
                else
                {
                    return AgregarNuevoAutor(tipo, nuevoNombre);
                }
            }

            return opciones[seleccion - 1].Id;
        }
        static int AgregarNuevoGenero(string tipo, string nuevoNombre)
        {
            using (var connection = new MySqlConnection(connectionString))
            {
                connection.Open();
                using (var command = new MySqlCommand("INSERT INTO Generos (genero) VALUES (@genero)", connection))
                {
                    command.Parameters.AddWithValue("@genero", nuevoNombre);
                    command.ExecuteNonQuery();
                }

                return ObtenerUltimoIdGenerado(connection);
            }
        }
        static int AgregarNuevoAutor(string tipo, string nuevoNombre)
        {
            using (var connection = new MySqlConnection(connectionString))
            {
                connection.Open();
                using (var command = new MySqlCommand("INSERT INTO Autor (Autor) VALUES (@autor)", connection))
                {
                    command.Parameters.AddWithValue("@autor", nuevoNombre);
                    command.ExecuteNonQuery();
                }
        ...
        static int ObtenerUltimoIdGenerado(MySqlConnection connection)
        {
        ...
        static void ActualizarGenero()
        {
            try
            {
                Console.Write("Ingrese el número del género que desea actualizar: ");
                int numeroGenero = int.Parse(Console.ReadLine());

                Console.Write("Ingrese el nuevo nombre del género: ");
                ...
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    string query = "UPDATE Generos SET genero = @nuevoGenero WHERE Numero_Genero = @numeroGenero";
                    MySqlCommand cmd = new MySqlCommand(query, conn);
                    ...
                    {
                        Console.WriteLine("No se encontró el género con ese número.");
                    }
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error al actualizar el género: {ex.Message}");
            }
        }
    }
}